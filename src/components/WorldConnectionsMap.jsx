...
export default function WorldConnectionsMap({ config = worldConnections }) {
  ...
  function countryKey(event, index) {
    ...
    } else if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); show(event, countries[index].properties.name, 'Pays', countries[index].id) }
  }
  ...
  return <section ref={section} className="world-connections section" id="monde" style={{ ...colors, '--world-pulse': `${pulse}s` }} data-running={running} aria-labelledby={`${uid}-heading`}>
    <div className="wrap">
      <div className="world-heading"><div><div className="eyebrow"><span /> UNE PERSPECTIVE MONDIALE</div><h2 id={`${uid}-heading`}>Un point de départ.<br /><em>Un monde de connexions.</em></h2></div><p>Depuis Casablanca, explorez des connexions à travers les continents. Une visualisation pour imaginer les échanges de demain.</p></div>
      {!validOrigin ? <p role="status">Carte indisponible : vérifiez les coordonnées de l’origine dans la configuration.</p> : <>
        <div className="world-toolbar"><span><Globe2 size={17} /> {origin.label}<small>ORIGINE · DÉMO</small></span><button type="button" onClick={() => setPaused(value => !value)} aria-pressed={paused} aria-label={paused ? 'Reprendre les animations' : 'Mettre les animations en pause'}>{paused ? <Play size={14} /> : <Pause size={14} />}<span>{paused ? 'Reprendre' : 'Pause'}</span></button></div>
        <svg className="world-svg" viewBox="0 0 1200 630" role="group" aria-labelledby={`${uid}-title ${uid}-desc`} onPointerLeave={() => { setTooltip(null); setActiveCountry(null) }}>
          <title id={`${uid}-title`}>Connexions mondiales depuis {origin.label}</title><desc id={`${uid}-desc`}>Carte du monde interactive. Lieux et routes de démonstration, sans couverture commerciale confirmée. Utilisez les flèches pour parcourir les pays, ou le sélecteur sous la carte.</desc>
          <defs><clipPath id={`${uid}-clip`}><path d={path({ type: 'Sphere' })} /></clipPath></defs>
          <g clipPath={`url(#${uid}-clip)`}>
            {countries.map((country, index) => <path ref={el => { countryRefs.current[index] = el }} key={country.id ?? index} d={path(country)} className={`world-country${country.id === originId ? ' is-origin' : ''}${country.id === activeCountry ? ' is-hovered' : ''}`} role="button" tabIndex={country.id === activeCountry || (!activeCountry && index === 0) ? 0 : -1} aria-label={country.properties.name} onPointerEnter={event => show(event, country.properties.name, 'Pays', country.id)} onFocus={event => show(event, country.properties.name, 'Pays', country.id)} onBlur={() => setTooltip(null)} onClick={event => show(event, country.properties.name, 'Pays', country.id)} onKeyDown={event => countryKey(event, index)} />)}
          ...
          {locations.map((location, index) => { const point = projection(location.coordinates); return <g key={location.id} transform={`translate(${point[0]} ${point[1]})`} className={`world-marker${index === 0 ? ' is-origin' : ''}`} role="button" tabIndex="0" aria-label={`${location.label} — ${index === 0 ? 'origine' : 'destination'} de démonstration`} onPointerEnter={event => show(event, location.label, 'Lieu de démonstration')} onFocus={event => show(event, location.label, 'Lieu de démonstration')} onBlur={() => setTooltip(null)} onClick={event => show(event, location.label, 'Lieu de démonstration')} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); show(event, location.label, 'Lieu de démonstration') } }}>
        ...
        <div className="world-explore"><label>Pays à explorer<select aria-label="Explorer un pays" value={activeCountry ?? ''} onChange={event => { const country = countries.find(item => item.id === event.target.value); if (country) show(event, country.properties.name, 'Pays', country.id) }}><option value="">Choisir un pays</option>{[...countries].sort((a, b) => a.properties.name.localeCompare(b.properties.name)).map((country, index) => <option key={country.id ?? index} value={country.id}>{country.properties.name}</option>)}</select></label><span>Survolez, touchez ou explorez au clavier.</span></div>
        <div className="world-locations" aria-label="Destinations de démonstration">{destinations.map(location => <button key={location.id} type="button" onFocus={event => show(event, location.label, 'Destination de démonstration')} onBlur={() => setTooltip(null)} onClick={event => show(event, location.label, 'Destination de démonstration')}><span />{location.label}</button>)}</div>
      </>}
      <div className="world-footnote"><span>Lieux et liaisons illustratifs · Ni bureaux, ni clients, ni couverture commerciale confirmée.</span><a href="https://www.naturalearthdata.com/about/terms-of-use/" target="_blank" rel="noreferrer">Géographie : Natural Earth</a></div>
    </div>
