...
export default function FeaturePage({ feature }) {
  ...
  return <div className="feature-page">
    <header className="feature-page-nav wrap"><a href="/" className="feature-page-brand">Grow <span>LOGISTICS</span></a><a href="/fonctionnalites"><ArrowLeft size={17}/> {t("Toutes les fonctionnalités")}</a><LanguageSwitch/></header>
    <main>
      <section className="feature-page-hero wrap">
        <div className="eyebrow" style={{'--open-index':0}}><span/> {feature.product} {t("· Fonctionnalité")}</div>
        <h1 style={{'--open-index':1}}>{feature.title}</h1>
        <p style={{'--open-index':2}}>{feature.intro}</p>
        <div className="feature-page-audience" style={{'--open-index':3}}>{t("Pour")} {feature.audience.toLocaleLowerCase(locale)}</div>
        <a href="#deroulement" className="btn btn-primary" style={{'--open-index':4}}>{t("Comprendre le fonctionnement")} <ArrowRight size={18}/></a>
      </section>
      <section className="feature-page-flow wrap" id="deroulement" aria-labelledby="flow-title">
        <div className="section-head" data-reveal><div className="eyebrow"><span/> {t("Pas à pas")}</div><h2 id="flow-title">{t("Ce qui se passe, concrètement.")}</h2><p>{t("Sélectionnez une étape pour découvrir son rôle dans vos opérations.")}</p></div>
        <div className="feature-flow-grid">
          ...
          <div className="feature-preview" id="feature-step-preview" data-reveal aria-live="polite" aria-atomic="true">
            <div className="feature-preview-top"><span>{feature.product}</span><small>{t("Exemple de déroulement")}</small></div>
            ...
            <button className="text-link" onClick={() => setActive((active + 1) % feature.steps.length)}>{active === 3 ? t("Revoir le déroulement") : t("Étape suivante")} <ArrowRight size={18}/></button>
          </div>
        </div>
      </section>
      <section className="feature-outcome" data-reveal><div className="wrap"><Check size={34}/><div><div className="eyebrow">{t("À l’arrivée")}</div><h2>{feature.result}</h2></div><a href="/contact" className="btn btn-primary">{t("Parlons de votre projet")} <ArrowRight size={18}/></a></div></section>
      <nav className="wrap feature-related" aria-label={t("Autres fonctionnalités")}><h2 data-reveal>{t("Découvrez aussi")}</h2><div>{featureDetails.filter(item => item.slug !== feature.slug).map((item, index) => <a key={item.slug} href={`/fonctionnalites/${item.slug}`} data-reveal style={{'--reveal-index':index}}><small>{item.product}</small><span>{item.title}</span><ArrowRight size={18}/></a>)}</div></nav>
    </main>
    <footer className="feature-page-footer wrap" data-reveal><span>© 2026 Grow Logistics</span><a href="/">{t("Retour à l’accueil")} <ArrowRight size={16}/></a></footer>
  </div>
}
